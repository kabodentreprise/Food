...
from app.models import User, FooterSettings
...
def update_user_admin_status(db: Session, user_id: int, is_admin: bool) -> Optional[User]:
    """Accorde ou retire le rôle d'administrateur à un utilisateur."""
    db_user = db.query(User).filter(User.id == user_id).first()
    if not db_user:
        return None
...
def update_user_super_admin_status(db: Session, user_id: int, is_super_admin: bool) -> Optional[User]:
    """Accorde ou retire le rôle de super administrateur à un utilisateur."""
    db_user = db.query(User).filter(User.id == user_id).first()
    if not db_user:
        return None
...
def update_user_livreur_status(db: Session, user_id: int, is_livreur: bool) -> Optional[User]:
    """Accorde ou retire le rôle de livreur à un utilisateur."""
    db_user = db.query(User).filter(User.id == user_id).first()
    if not db_user:
        return None
